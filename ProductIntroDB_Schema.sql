...
use ProductIntro
go
-- 1: Tạo Table [Account] chứa tài khoản thành viên trong hệ thống ----
create table Account(
	...
	phone nvarchar(20) not null,			-- Only digits, begin with 03|05|07|08|09
	isActive bit not null,		-- 1: being used | 0: is prevented
	[role] int,	-- Role in system {1: admin - others: staff}
	isDeleted bit default 0
)
go
-- Step 1: Alter the isActive column to allow NULL values
ALTER TABLE Account
ALTER COLUMN isActive bit NULL;

-- Step 2: Set the default value of isActive to 0
ALTER TABLE Account
ADD CONSTRAINT DF_Account_isActive DEFAULT 0 FOR isActive;

ALTER TABLE Account
ADD CONSTRAINT UQ_Account_Phone UNIQUE (phone);

ALTER TABLE Account ADD dob_temp date;
UPDATE Account SET dob_temp = CAST(dob AS date);
ALTER TABLE Account DROP COLUMN dob;
EXEC sp_rename 'Account.dob_temp', 'dob', 'COLUMN';

-- 2: Tạo Table [Category] chứa thông tin loại sản phẩm, ngành hàng -----------------------------
create table Category(
	categoryId int primary key not null identity,
	categoryName nvarchar(88) not null unique,
	note ntext default ''
)
go
-- 3: Tạo Table [Product] chứa thông tin của sản phẩm cần giới thiệu --------------
create table Product(
	...
	username varchar(20) not null foreign key references Account(username) on update cascade,
...
insert into Account(username, [password], lastName, firstName, dob, gender, phone, isActive, [role]) 
values('quangpro','123',N'Nguyễn Minh','Quang','1996/06/12', 1, '0935694223',1, 2)
insert into Account(username, [password], lastName, firstName, dob, gender, phone, isActive, [role]) 
values('hungnguyen','abc',N'Nguyễn Quang',N'Hưng','1996/10/28', 1, '0705101028',1, 1)

insert into Account(username, [password], lastName, firstName, dob, gender, phone, [role]) 
values('nhatthang27','789',N'Trần Nhật',N'Thắng','2004/04/27', 1, '0705101028', 1)
GO

insert into Category(categoryName) values(N'Dụng cụ nhà bếp')
insert into Category(categoryName) values(N'Điện gia dụng')
insert into Category(categoryName) values(N'Trang trí nội thất')
insert into Category(categoryName) values(N'Dụng cụ thể thao')
insert into Category(categoryName) values(N'Thiết bị thông minh')
insert into Category(categoryName) values(N'Quần - Áo thời trang')
go

insert into Product (productId, productName, productImage, brief, username, price, discount, categoryId, unit)		-- => OK
